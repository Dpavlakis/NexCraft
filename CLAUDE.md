...
4c. **Breadcrumb + nav polish — DONE on `test`, NOT yet eyeballed** (made after the last Test rebuild): `Breadcrumbs.vue` now uses `router.push` (fixed the full-page-reload / blank "Rendering Application…" screen); breadcrumb root "Control Panel"→"Dashboard"; top-nav text now semibold (600) in `AppHeader.vue`. Convention note: per-instance manage-popup cards hide Return+title via `embeddedInManageModal` (see Conventions).
5. **Queued / next:**
