...
- **Import an existing server** (`feat/import-existing-server`, DONE — pending final user test on the Test stack, then merge to main): zip upload → auto-detect (`server_detect.ts`), admin-only. **Java = "Import as-is" only** (run the uploaded server with its world/mods; the pack-reinstall/find-my-modpack modes were removed). **Bedrock = "Install latest BDS + keep world"** (version-lock) with an import-as-is opt-out + forced start cmd + chmod. Entry is an **"Import / Existing" source tab** in the modpack browser (the top Create-Instance cards were removed). Wizard: `frontend/src/widgets/setupApp/ImportServerReview.vue`; daemon `import_router.ts`; panel `import_router.ts` (`/protected_import`).

## In-flight / next session (checkpoint 2026-05-31)
Chat got too long/slow → checkpointed here. Resume order:
1. **World Management (#15)** — design APPROVED, spec at `docs/superpowers/specs/2026-05-31-world-management-design.md`. Active-world model: new per-instance card `World.vue` (`/instances/world`, admin-gated). Actions: info / download (Java `<level-name>.zip` of world+_nether+_the_end; Bedrock `.mcworld` of `worlds/<name>` contents) / replace-restore (upload → auto world-backup → wipe → unzip w/ `findWorldRoot` level.dat detection) / reset (auto-backup → remove → regen). Auto-backup-the-world (world-only zip into Backups area, restore-compatible) on **replace & reset only**; download none. Stop-then-restore-run-state for destructive ops. Reuse chunked upload+unzip, signed-token download, backupManager/archiver, `readLevelName`, node-stream-zip. **Next step: `writing-plans` → subagent-driven execution.**
2. **Modpack-browser UI polish** (import branch): (a) rename the **"Custom" tab → "Vanilla"** + add a **clean (non-trademark) grass-block SVG** to `frontend/src/assets/` as its tab icon; (b) **Import tab/panel icon → Google Material Icons "File Upload"** glyph (bundle SVG in assets/); (c) **enrich the non-modpack install-detail dialog** (Vanilla/Paper/Purpur/Folia/Fabric/Forge/NeoForge/Quilt/Bedrock) — currently bland (just version + one sentence) vs the rich modpack detail; add software blurb/description, logo, key features, Java-version requirement, links (project page / "Learn more"), release date/tags. *(c is a small feature → brainstorm/spec first.)*
...
4. **Test deployment stack** is live in parallel: `:test` image tag, Test Unraid templates (`unraid/nexcraft-{web,daemon}-test.xml`), isolated data `/mnt/user/appdata/nexcraft-test`, TEST-badged logo. Prod on .41/.42, **Test on .46/.47**. The user triggers the publish workflow (don't extract the GitHub token — safety-blocked). On merge of import: re-take the branch version of `frontend/src/widgets/market/index.vue` (drop the temporary hidden-cards patch) and repoint Test template `<Icon>`/`<TemplateURL>` from the branch URL to main.
