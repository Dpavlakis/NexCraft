...
4. **#30 Richer non-modpack install detail — DONE on `test`**: the custom-builder Details tab (all 9 software) now shows a tags row (version + release type + category), a curated **Key Features** list, and a **"Java auto-provisioned"** note (all except Bedrock). Curated content lives in `frontend/src/tools/loaderInfo.ts` (`categoryKeys`/`featureKeys`) + `en_US.json`; rendered in `ModpackBrowser.vue`. Spec/plan under `docs/superpowers/`. (Java requirement is the auto-provision note — no version heuristic, accurate for all versions.)
5. **Queued / next:**
