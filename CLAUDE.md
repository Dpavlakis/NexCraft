...
5. **Queued / next (start on `test`):**
   - **Appearance settings page cleanup** (`frontend/src/widgets/Settings.vue`): (a) **remove the "Custom Layout" block** — the `<a-form-item>` at ~lines 516–536 with the `startDesignUI()` "Start Designing" button (i18n keys `ebd2a6a1`/`ba717ff3`/`bc46c15b`/`3b24a247`); custom layout was removed everywhere else. ⚠️ Also check `frontend/src/hooks/useHeaderMenus.ts` which references `TXT_CODE_ebd2a6a1` (the design-mode nav entry) — remove/guard that too. (b) **Resize the "Custom Text on the Login Page" textarea** (`<a-textarea v-model:value="formData.loginInfo">` ~line 545) to match the width of the image/element above it — the bgUrl input uses `style="max-width: 320px"` (~line 574); apply the same so it lines up.
   ...
   - **Optional:** trim `CreateInstanceForm.vue`'s now-dead DOCKER/EXIST/FILE branches (left from #18).
